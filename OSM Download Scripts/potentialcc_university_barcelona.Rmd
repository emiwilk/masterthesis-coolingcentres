...
barcelona_university_query <- opq(bbox = barcelona_bbox) %>%
  add_osm_feature(key = "amenity", value = "university") %>%
  osmdata_sf()

# Extract points and polygons
university_points <- barcelona_university_query$osm_points
university_polygons <- barcelona_university_query$osm_polygons
...
if (!is.null(university_points) && nrow(university_points) > 0) {
  ...
  university_points_clean <- sf::st_sf(data.frame(), crs = st_crs(barcelona_districts))
}
```
...
if (!is.null(university_polygons) && nrow(university_polygons) > 0) {
  ...
  university_polygons_clean <- sf::st_sf(data.frame(), crs = st_crs(barcelona_districts))
}
```
...
if (nrow(university_points_clean) > 0 && nrow(university_polygons_clean) > 0) {
  ...
  barcelona_university <- bind_rows(
    university_points_clean[common_cols],
    university_polygons_clean[common_cols]
  )
} else if (nrow(university_points_clean) > 0) {
  barcelona_university <- university_points_clean
} else if (nrow(university_polygons_clean) > 0) {
  barcelona_university <- university_polygons_clean
} else {
  barcelona_university <- sf::st_sf(data.frame(), crs = st_crs(barcelona_districts))
}
```
...
if (nrow(barcelona_university) > 0) {
  # First step: Remove rows where name is NA
  barcelona_university_no_na <- barcelona_university %>%
    filter(!is.na(name))
  
  # Check if we still have data after NA filtering
  if (nrow(barcelona_university_no_na) > 0) {
    # Separate points and polygon centroids
    university_points <- barcelona_university_no_na %>%
      filter(university_type == "point")
    
    university_polygons <- barcelona_university_no_na %>%
    ...
    if (nrow(university_points_filtered) > 0 && nrow(university_polygons_filtered) > 0) {
      barcelona_university_final <- bind_rows(university_points_filtered, university_polygons_filtered)
    } else if (nrow(university_points_filtered) > 0) {
      barcelona_university_final <- university_points_filtered
    } else if (nrow(university_polygons_filtered) > 0) {
      barcelona_university_final <- university_polygons_filtered
    } else {
      # Create empty sf object if no data remains
      barcelona_university_final <- st_sf(
...
cat("Total university found:", nrow(barcelona_university_final), "\n")
if (nrow(barcelona_university_final) > 0) {
  cat("university by type:\n")
  print(table(barcelona_university_final$university_type, useNA = "ifany"))
  
  cat("\nuniversity by operator type:\n")
  print(table(barcelona_university_final$operator.type, useNA = "ifany"))
  
  cat("\nuniversity with names:", sum(!is.na(barcelona_university_final$name)), "\n")
}
```
...
if (nrow(barcelona_university_final) > 0) {
  ...
  display_cols <- intersect(c("osm_id", "name", "amenity", "operator", "operator.type", 
                            ...
                            names(barcelona_university_final))
  
  barcelona_university_display <- barcelona_university_final %>%
    st_drop_geometry() %>%
    select(all_of(display_cols)) %>%
    head(10)
  
  print(barcelona_university_display)
}
```
...
if (nrow(barcelona_university_final) > 0) {
  ...
  university_wgs84 <- st_transform(barcelona_university_final, 4326)
...
if (nrow(barcelona_university_final) > 0) {
  ...
  barcelona_university_table <- barcelona_university_final %>%
    # Extract coordinates
    mutate(
      x = st_coordinates(.)[,1],
      y = st_coordinates(.)[,2]
  ...
  write.csv(barcelona_university_table, "Data/potential cooling centres/barcelona_university.csv", row.names = FALSE)
  
  cat("Data saved as barcelona_university.rds and barcelona_university.csv\n")
