...
athens_university_query <- opq(bbox = athens_bbox) %>%
  add_osm_feature(key = "amenity", value = "university") %>%
  osmdata_sf()

# Extract points and polygons
university_points <- athens_university_query$osm_points
university_polygons <- athens_university_query$osm_polygons
...
if (!is.null(university_points) && nrow(university_points) > 0) {
  ...
  university_points_clean <- sf::st_sf(data.frame(), crs = st_crs(athens_districts))
}
```
...
if (!is.null(university_polygons) && nrow(university_polygons) > 0) {
  ...
  university_polygons_clean <- sf::st_sf(data.frame(), crs = st_crs(athens_districts))
}
```
...
if (nrow(university_points_clean) > 0 && nrow(university_polygons_clean) > 0) {
  ...
  athens_university <- bind_rows(
    university_points_clean[common_cols],
    university_polygons_clean[common_cols]
  )
} else if (nrow(university_points_clean) > 0) {
  athens_university <- university_points_clean
} else if (nrow(university_polygons_clean) > 0) {
  athens_university <- university_polygons_clean
} else {
  athens_university <- sf::st_sf(data.frame(), crs = st_crs(athens_districts))
}
```
...
if (nrow(athens_university) > 0) {
  # First step: Remove rows where name is NA
  athens_university_no_na <- athens_university %>%
    filter(!is.na(name))
  
  # Check if we still have data after NA filtering
  if (nrow(athens_university_no_na) > 0) {
    # Separate points and polygon centroids
    university_points <- athens_university_no_na %>%
      filter(university_type == "point")
    
    university_polygons <- athens_university_no_na %>%
    ...
    if (nrow(university_points_filtered) > 0 && nrow(university_polygons_filtered) > 0) {
      athens_university_final <- bind_rows(university_points_filtered, university_polygons_filtered)
    } else if (nrow(university_points_filtered) > 0) {
      athens_university_final <- university_points_filtered
    } else if (nrow(university_polygons_filtered) > 0) {
      athens_university_final <- university_polygons_filtered
    } else {
      # Create empty sf object if no data remains
      athens_university_final <- st_sf(
...
cat("Total university found:", nrow(athens_university_final), "\n")
if (nrow(athens_university_final) > 0) {
  cat("university by type:\n")
  print(table(athens_university_final$university_type, useNA = "ifany"))
  
  cat("\nuniversity by operator type:\n")
  print(table(athens_university_final$operator.type, useNA = "ifany"))
  
  cat("\nuniversity with names:", sum(!is.na(athens_university_final$name)), "\n")
}
```
...
if (nrow(athens_university_final) > 0) {
  ...
  display_cols <- intersect(c("osm_id", "name", "amenity", "operator", "operator.type", 
                            ...
                            names(athens_university_final))
  
  athens_university_display <- athens_university_final %>%
    st_drop_geometry() %>%
    select(all_of(display_cols)) %>%
    head(10)
  
  print(athens_university_display)
}
```
...
if (nrow(athens_university_final) > 0) {
  ...
  university_wgs84 <- st_transform(athens_university_final, 4326)
...
if (nrow(athens_university_final) > 0) {
  ...
  athens_university_table <- athens_university_final %>%
    # Extract coordinates
    mutate(
      x = st_coordinates(.)[,1],
      y = st_coordinates(.)[,2]
  ...
  write.csv(athens_university_table, "Data/potential cooling centres/athens_university.csv", row.names = FALSE)
  
  cat("Data saved as athens_university.rds and athens_university.csv\n")
  cat("CSV contains columns: osm_id, name, amenity, operator, addr.postcode, x, y\n")
}
```
