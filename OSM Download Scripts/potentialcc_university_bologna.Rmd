...
bologna_university_query <- opq(bbox = bologna_bbox) %>%
  add_osm_feature(key = "amenity", value = "university") %>%
  osmdata_sf()

# Extract points and polygons
university_points <- bologna_university_query$osm_points
university_polygons <- bologna_university_query$osm_polygons
...
if (!is.null(university_points) && nrow(university_points) > 0) {
  ...
  university_points_clean <- sf::st_sf(data.frame(), crs = st_crs(bologna_districts))
}
```
...
if (!is.null(university_polygons) && nrow(university_polygons) > 0) {
  ...
  university_polygons_clean <- sf::st_sf(data.frame(), crs = st_crs(bologna_districts))
}
```
...
if (nrow(university_points_clean) > 0 && nrow(university_polygons_clean) > 0) {
  ...
  bologna_university <- bind_rows(
    university_points_clean[common_cols],
    university_polygons_clean[common_cols]
  )
} else if (nrow(university_points_clean) > 0) {
  bologna_university <- university_points_clean
} else if (nrow(university_polygons_clean) > 0) {
  bologna_university <- university_polygons_clean
} else {
  bologna_university <- sf::st_sf(data.frame(), crs = st_crs(bologna_districts))
}
```
...
if (nrow(bologna_university) > 0) {
  # First step: Remove rows where name is NA
  bologna_university_no_na <- bologna_university %>%
    filter(!is.na(name))
  
  # Check if we still have data after NA filtering
  if (nrow(bologna_university_no_na) > 0) {
    # Separate points and polygon centroids
    university_points <- bologna_university_no_na %>%
      filter(university_type == "point")
    
    university_polygons <- bologna_university_no_na %>%
    ...
    if (nrow(university_points_filtered) > 0 && nrow(university_polygons_filtered) > 0) {
      bologna_university_final <- bind_rows(university_points_filtered, university_polygons_filtered)
    } else if (nrow(university_points_filtered) > 0) {
      bologna_university_final <- university_points_filtered
    } else if (nrow(university_polygons_filtered) > 0) {
      bologna_university_final <- university_polygons_filtered
    } else {
      # Create empty sf object if no data remains
      bologna_university_final <- st_sf(
...
cat("Total university found:", nrow(bologna_university_final), "\n")
if (nrow(bologna_university_final) > 0) {
  cat("university by type:\n")
  print(table(bologna_university_final$university_type, useNA = "ifany"))
  
  cat("\nuniversity by operator type:\n")
  print(table(bologna_university_final$operator.type, useNA = "ifany"))
  
  cat("\nuniversity with names:", sum(!is.na(bologna_university_final$name)), "\n")
}
```
...
if (nrow(bologna_university_final) > 0) {
  ...
  display_cols <- intersect(c("osm_id", "name", "amenity", "operator", "operator.type", 
                            ...
                            names(bologna_university_final))
  
  bologna_university_display <- bologna_university_final %>%
    st_drop_geometry() %>%
    select(all_of(display_cols)) %>%
    head(10)
  
  print(bologna_university_display)
}
```
...
if (nrow(bologna_university_final) > 0) {
  ...
  university_wgs84 <- st_transform(bologna_university_final, 4326)
...
if (nrow(bologna_university_final) > 0) {
  ...
  bologna_university_table <- bologna_university_final %>%
    # Extract coordinates
    mutate(
      x = st_coordinates(.)[,1],
      y = st_coordinates(.)[,2]
  ...
  write.csv(bologna_university_table, "Data/potential cooling centres/bologna_university.csv", row.names = FALSE)
  
  cat("Data saved as bologna_university.rds and bologna_university.csv\n")
